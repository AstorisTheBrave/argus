...
def _event(guild_id: str, command: str, duration_ms: float) -> dict[str, object]:
    return {
...
async def test_sink_insert_and_query_roundtrip() -> None:
    ...
    try:
        await sink.record(_event("99", "ping", 10.0))
        await sink.record(_event("99", "ping", 30.0))
        await sink.record(_event("99", "weather", 100.0))
