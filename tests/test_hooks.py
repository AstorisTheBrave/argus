"""Hook wiring, counter movement, fail-open behaviour (D4 gate)."""
...
async def test_on_interaction_counts_received(env: Env) -> None:
    ...
    assert (
        env.backend.count(env.names.interactions_total, type="modal_submit", status="received") == 1
    )
...
async def test_app_command_completion_success_and_duration(env: Env) -> None:
    ...
    assert env.backend.count(env.names.app_commands_total, command="ping", status="success") == 1
...
async def test_app_command_error_counts_error_and_error_type(env: Env) -> None:
    ...
    assert env.backend.count(env.names.app_commands_total, command="ping", status="error") == 1
    assert (
        env.backend.count(env.names.command_errors_total, command="ping", error_type="ValueError")
        == 1
    )
...
async def test_prefix_command_completion_and_error(env: Env) -> None:
    ...
    assert env.backend.count(env.names.commands_total, command="sync", status="success") == 1
    assert env.backend.count(env.names.commands_total, command="sync", status="error") == 1
    assert (
        env.backend.count(env.names.command_errors_total, command="sync", error_type="KeyError")
        == 1
    )
...
async def test_gateway_and_shard_events(env: Env) -> None:
    ...
    assert env.backend.count(env.names.gateway_events_total, event="MESSAGE_CREATE") == 1
    assert env.backend.count(env.names.shard_reconnects_total, shard="3") == 2  # connect + resume
    assert env.backend.count(env.names.shard_disconnects_total, shard="3") == 1
...
async def test_hook_failure_is_counted_and_swallowed(env: Env) -> None:
    ...
    assert env.backend.count(env.names.instrumentation_errors_total, hook="on_interaction") == 1
...
def test_log_handler_counts_records_and_ratelimits(env: Env) -> None:
    ...
    try:
        ...
        assert (
            env.backend.count(env.names.log_records_total, logger="discord.http", level="WARNING")
            == 1
        )
        assert env.backend.count(env.names.ratelimits_total) == 1
    finally:
        reg.remove()
